...
export class RoleBuilder {
    ...
    grant(actionName: string, scope: Scope): RoleBuilder {
        if (this.role.actions.filter((p) => p.name === actionName).length > 0) {
            throw new Error(
                `Action with name '${actionName} already exists on role ${this.role.name}`
            );
        }
    ...
    inherit(role: string): RoleBuilder {
        if (this.role.inherits.filter((r) => r === role).length > 0) {
            throw new Error(
                `Role with name '${role} is already inherited by role ${this.role.name}`
            );
        }
    ...
    static roleDefinitionToRoles(
        ...
        for (const roleName in roleDefinitions) {
            ...
            if (rolesAndActions.length !== 2) {
                throw new Error(
                    `Invalid role definition, there must be exactly one ';' between roles and actions: ${roleDefinition}`
                );
            }
            const inheritedRoleNames: string[] = rolesAndActions[0].split(",");
            role.inherits.push(
                ...inheritedRoleNames.filter((s) => s.trim() !== "")
            );
            ...
            for (const action of actions.filter((s) => s.trim() !== "")) {
                ...
                if (actionWithScope.length !== 2) {
                    throw new Error(
                        "Invalid action definition, there must be exactly one ':' between name and scope"
                    );
                }
        ...
        for (const role of roles) {
            ...
            for (const inheritedRole of role.inherits) {
                if (!validatedRoles.has(inheritedRole)) {
                    throw new Error(
                        `Role '${role.name}' inherits from '${inheritedRole}', but this role does not exist!`
                    );
                }
            }
            ...
            for (const action of role.actions) {
                if (actionSet.has(action.name)) {
                    throw new Error(
                        `Role '${role.name}' has a duplicate action: '${action.name}'`
                    );
                }
