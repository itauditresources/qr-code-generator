...
export class RBAC {
    ...
    constructor(roles?: RoleDefinitions | Role[], scopes?: Scope[]) {
        if (scopes && scopes.length < 1) {
            throw new Error(
                "At least one scope must be given. Omit the value to use a set of default scopes."
            );
        }
        if (!scopes) {
            for (const scopeName in DEFAULT_SCOPES) {
                this.scopeMap.set(
                    DEFAULT_SCOPES[scopeName].name,
                    DEFAULT_SCOPES[scopeName]
                );
            }
        } else {
        ...
        if (roles && !Array.isArray(roles)) {
            for (const role of RoleDefinitionsHelper.roleDefinitionToRoles(
                this.scopeMap,
                roles
            )) {
    ...
    can(roles: string | string[]): PermissionChecker {
        ...
        return new PermissionChecker(
            this.scopeMap,
            this.getAllowedActionsForRoles(rolesAsArray)
        );
    }
    ...
    private resolveInheritedRoles(role: Role): Role[] {
        ...
        for (const inheritedRoleName of role.inherits) {
            ...
            for (const resolvedInheritedRole of this.resolveInheritedRoles(
                inheritedRole
            )) {
    ...
    getAllowedActionsForRoles(roleNames: string[]): Action[] {
        ...
        for (const roleName of roleNames) {
            ...
            const inheritedActions: Action[] = inheritedRoles
                .map((r) => r.actions)
                .reduce(
            ...
            for (const newAction of allActions) {
                if (!allowedActions.has(newAction.name)) {
                    ...
                    if (
                        existingAction &&
                        existingAction.scope.weight < newAction.scope.weight
                    ) {
                        allowedActions.set(newAction.name, newAction);
                    }
                }
