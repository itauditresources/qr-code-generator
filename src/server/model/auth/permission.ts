...
export class PermissionChecker {
    private readonly allowedActions: Map<string, Action> = new Map<
        string,
        Action
    >();
    ...
    do(action: string, requestedScope?: Scope): Permission {
        ...
        const allowedAction: Action | undefined =
            this.allowedActions.get(action);

        const evaluatedScopes: Map<string, boolean> = new Map<
            string,
            boolean
        >();
        this.scopes.forEach((value) => evaluatedScopes.set(value.name, false));

        const denied = new Permission(requestedAction, false, evaluatedScopes);
        ...
        const granted = new Permission(requestedAction, true, evaluatedScopes);
